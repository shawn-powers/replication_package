********************************************************************************
********************************************************************************
************* Construction of Panel Data Sets - Randomization ******************
********************************************************************************
********************************************************************************
...
********************************************************************************
*				      Fortis Charging Data - 06_19_2023				    	   *				
********************************************************************************

// Vehicles Information
import excel "$fortisdata/Fortis Data Transfer - 06_19_2023 UCalgary.xlsx", sheet("Vehicle Data") firstrow clear
drop date_joined
save "$data/vehicle_data_randomization.dta", replace

// CHARGING SESSIONS DATA
import excel "$fortisdata/Fortis Data Transfer - 06_19_2023 UCalgary.xlsx", sheet("Charge Event Data") firstrow clear
...
drop start_time end_time car_scheduled_departure_time   

save "$data/charge_event_randomization.dta", replace

********************************************************************************
*				      Fortis Interval Data - 06_19_2023				    	   *				
********************************************************************************

import delimited "$fortisdata/Fortis Interval Data - 06_19_2023 UCalgary.csv" ,  asdouble clear 
...
duplicates t vehicle_id charge_id timestamp, gen(dup)
...
merge m:1 vehicle_id charge_id using "$data/charge_event_randomization.dta"
drop if _merge==2 // Obs in the other category (data from charge event sheet that are not matched) have zero kwh added
drop _merge


merge m:1 vehicle_id  using "$data/vehicle_data_randomization.dta"
drop if _merge == 2 // EVs with no charge data 
drop _merge
...
save "$panel/panel_10min_randomization.dta" , replace
...
use "$panel/panel_10min_randomization.dta" , clear
gen hour=hh(from)
gen date=dofc(from)
format date %td
...
bys vehicle_id charge_id date hour (from) : egen median_amp_h=median(charge_amps) 
label var median_amp_h "Hour's Median Amp"
bys vehicle_id charge_id date hour (from) : egen median_voltage_h=median(charger_voltage) 
label var median_voltage_h "Hour's Median Volt"

bys vehicle_id charge_id date hour (from) : egen max_amp_h=max(charge_amps) 
label var max_amp_h "Hour's Max Amp"
bys vehicle_id charge_id date hour (from) : egen max_voltage_h=max(charger_voltage) 
label var max_voltage_h "Hour's Max Volt"

bys vehicle_id charge_id date hour (from) : egen double kWh_added_hour=total(kwh_added_to_car_battery)
bys vehicle_id charge_id date hour (from) : egen double range_added_hour=total(range_added)
bys vehicle_id charge_id date hour (from) : egen double batterylvl_added_hour=total(battery_level_added)
...
drop from to  datetime_utc kwh_added_to_car_battery range_added battery_level_added charge_amps charger_voltage date hour median_amp_10m max_amp_10m median_voltage_10m max_voltage_10m 

order vehicle_id charge_id time median_amp_h median_voltage_h max_amp_h max_voltage_h kWh_added_hour range_added_hour batterylvl_added_hour location charger_type make model year est_battery_range_miles tesla_powerwall tesla_solar s_start s_end st_duration_hours st_duration_minutes

save "$panel/panel_hourly_randomization.dta", replace
